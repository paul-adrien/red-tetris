...
export class pieceService implements OnDestroy {
    ...
    constructor(private routes: Router,
        ...
        this.socketService.listenToServer('res start piece').subscribe((data) => {
            if (data.piece.id === this.pieceName) {
                ...
                this.newTetro = true;
                this.currentTetro = 0;
                console.log('start', this.tetroList, this.playersInGame);
                //this.timerInterval()
            }
        });
        ...
        this.socketService.listenToServer('res new tetrominos').subscribe((data) => {
            if (data.piece.id === this.pieceName) {
                //this.tetroList = data.piece.tetroList;
        ...
        this.socketService.listenToServer('res malus').subscribe((data) => {
            console.log(data)
            if (data.pieceId === this.pieceName && this.player.name !== data.playerName)
    ...
    //         console.log('test')
    //         //this.socketService.emitToServer('send spectrum', { pieceId: this.pieceName, id: this.socketService.socket.id });
    //     }), 650);
    // }
    ...
    startGame() {
        this.socketService.emitToServer('start piece', { pieceId: this.pieceName, playerName: this.player.name, id: this.socketService.socket.id });
    }
    ...
    lineClear(spectrum) {
        ...
        if (i > 1) {
            this.socketService.emitToServer('malus', { pieceId: this.pieceName, playerName: this.player.name, nbMalus: i });
            if (i == 2) {
                this.score += 250;
            } else if (i == 3) {
                this.score += 550;
    ...
    unDrawTetro(spectrum: [][], tetro) {
        spectrum.forEach((row: any, y) => {
            row.forEach((col: any, x) => {
                if (y >= tetro.position.y && y < tetro.position.y + 4
                    && x >= tetro.position.x && x < tetro.position.x + 4
    ...
    checkNewPlace(spectrum, tetro, moveY, moveX) {
        return tetro.tetro.every((row, y) => {
            return row.every((col, x) => {
                ...
                if (col == 0
                    || (spectrum[posY] && spectrum[posY][posX] != undefined && spectrum[posY][posX] === 0))
                    return true;
                else
                    return false;
            });
    ...
    rotate(tetro) {
        ...
        if (this.checkNewPlace(this.player.game.spectrum, this.tetroList[this.currentTetro], 0, 0)) {
            console.log('true')
            this.DrawNewTetro(this.player.game.spectrum, this.tetroList[this.currentTetro]);
            return;
        }
        else {
            console.log('false')
    ...
    move(event) {
        if (this.start == true && this.lock == false) {
            if (event === 'left') {
                this.unDrawTetro(this.player.game.spectrum, this.tetroList[this.currentTetro]);
                if (this.checkNewPlace(this.player.game.spectrum, this.tetroList[this.currentTetro], 0, -1))
                ...
                this.unDrawTetro(this.player.game.spectrum, this.tetroList[this.currentTetro]);
                if (this.checkNewPlace(this.player.game.spectrum, this.tetroList[this.currentTetro], 1, 0)) {
                ...
                this.unDrawTetro(this.player.game.spectrum, this.tetroList[this.currentTetro]);
                let yMax = this.maxDown(this.player.game.spectrum, this.tetroList[this.currentTetro]);
                this.score += (2 * yMax);
                this.tetroList[this.currentTetro].position.y += yMax - (this.malus > 0 ? 1 : 0);
                this.DrawNewTetro(this.player.game.spectrum, this.tetroList[this.currentTetro]);
            }
            this.sendSpectrum();
        }
    ...
    sendSpectrum() {
        this.socketService.emitToServer('send spectrum', { pieceId: this.pieceName, player: this.player, id: this.socketService.socket.id });
    }

    async leavePiece() {
        if (this.pieceName) {
            {
                this.socketService.emitToServer('leave piece', { pieceId: this.pieceName, playerName: this.player.name, id: this.socketService.socket.id })
            }
            this.pieceName = "";
            this.piecePlayers = [];
            this.pieceCreator = "";
