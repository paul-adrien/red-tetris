    ...
    io.on('connection', (socket) => {
        ...
        socket.on('check piece id', (data) => {
            if (data && data.pieceId && data.id) {
                return pieceController.checkPieceId(data.pieceId).then(
                    (res) => {
                    ...
                    pieceController.checkPlayerId(data.playerName).then(
                        (res2) => {
                            if (res2 === true && res1 === true) {
                                return pieceController.createPiece(data).then((res3) => {
                                    if (res3.newPiece === true)
                                        io.emit('updatePiece', { piece: res3.piece });
                                    if (res3.newPlayer === true)
                                        io.emit('updatePlayer', { piece: res3.piece, player: res3.player });
                                ...
                                if (res1 === false)
                                    io.to(data.id).emit('res check piece id', res1);
                            }
                        }
                    )
                }
            ...
            pieceController.checkPlayerId(data.playerName).then(
                (res1) => {
                    if (res1 === true) {
                        ...
                        console.log(res1)
                        if (res1 === false)
                            io.to(data.id).emit('res check player join id', { status: res1, index: data.index });
                    }
                }
            )
        });
        ...
        socket.on('piece list', async (data) => {
            return pieceController.pieceList().then((res) => {
        ...
        socket.on('start piece', async (data) => {
            ...
            return pieceController.startPiece(data).then((res) => {
        ...
        socket.on('player lose', async (data) => {
            ...
            return pieceController.playerLose(data).then((res) => {
                io.emit('res player lose', res);
            }).catch(err => { console.log(err) });
        });

        socket.on('new tetrominos', async (data) => {
            return pieceController.newTetrominos(data).then((res) => {
                io.emit('res new tetrominos', res);
            }).catch(err => { console.log(err) });
        });

        socket.on('send spectrum', async (data) => {
            return pieceController.updateSpectrum(data).then((res) => {
        ...
        socket.on('check player id', (data) => {
            if (data && data.playerId) {
                return pieceController.checkPlayerId(data.playerId).then(
                    (res) => {
        ...
        socket.on('player list', async (data) => {
            return pieceController.playerList().then((res) => {
